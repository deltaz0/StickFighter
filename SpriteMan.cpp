...
void CSpriteManager::Draw(ObjectType object, D3DXVECTOR3 p, float angle, float scale, int frame){ //draw sprite
  if(m_pSprite[object])
    m_pSprite[object]->Draw(p, angle, scale, frame);
}
...
void CSpriteManager::Draw(ObjectType object, D3DXVECTOR3 p, float angle, float scale, int frame, int facing){ //draw sprite
  if(m_pSprite[object])
    m_pSprite[object]->Draw(p, angle, scale, frame, facing);
}

void CSpriteManager::DrawDetectionBox(ObjectType object, D3DXVECTOR3 p, float scaleX, float scaleY, float xOffset, float yOffset){
    if(m_pSprite[object])
      m_pSprite[object]->DrawDetectionBox(p, scaleX, scaleY, xOffset, yOffset);
  }
