...
class C3DSprite{ //3D sprite
  ...
  private:
    ...
    int m_nCurrentOffset; // Index for current offset.
  ...
  public:
    ...
    void Draw(D3DXVECTOR3 p, float angle, float scale, int frame); ///< Draw sprite at point p in 3D space.
    void Draw(D3DXVECTOR3 p, float angle, float scale, int frame, int facing); ///< Draw sprite at point p in 3D space.
    void DrawDetectionBox(D3DXVECTOR3 p, float scaleX, float scaleY, float xOffset, float yOffset); //Draw detection box.

    void Release(); ///< Release sprite.z
