...
/// Constructor
CRenderer::CRenderer():m_d3d(NULL), m_d3ddevice(NULL), 
  m_pCursorBitmap(NULL), m_d3dbackbuffer(NULL){
}
...
BOOL CRenderer::ChooseGraphicsMode(D3DDISPLAYMODE& chosenmode){
  ...
  for(int i=0; i<nNumGraphicsModes && !bChosenMode; i++){ //for each mode
    ...
    if( curmode.Width==g_nScreenWidth && curmode.Height==g_nScreenHeight ){ //right size
        chosenmode = curmode; 
        bChosenMode = TRUE;
        testInt = curmode.RefreshRate;
      }
  }

	chosenmode.RefreshRate = 10;
  return bChosenMode;
} //ChooseGraphicsMode

void CRenderer::LoadMouseCursor(){
  HRESULT hres;
  hres = m_d3ddevice->CreateOffscreenPlainSurface(
  ...
  hres = D3DXLoadSurfaceFromFile(m_pCursorBitmap, NULL, NULL, //copy to surface
    g_cImageFileName[CURSOR_IMAGEFILE], NULL, D3DX_DEFAULT, 0, NULL);
}

void CRenderer::SetMouseCursor(){
  HRESULT hres = m_d3ddevice->SetCursorProperties(7, 0, m_pCursorBitmap);
}
...
BOOL CRenderer::InitD3D(HINSTANCE hInstance, HWND hwnd){ 
  ...
  hr=CreateD3DDevice(hwnd, displaymode);  //fps limiter! display mode
  if(FAILED(hr)){ //bail if failed
    m_d3ddevice = NULL; return FALSE;
  }

  SetD3dStates(FALSE); //orthogonal projection
...
HRESULT CRenderer::CreateD3DDevice(HWND hwnd, D3DDISPLAYMODE mode){ //create d3d device
  ...
  m_d3dpp.FullScreen_RefreshRateInHz = D3DPRESENT_RATE_DEFAULT;
	...
	int test = D3DPRESENT_INTERVAL_IMMEDIATE;
	int testtwo = D3DPRESENT_RATE_DEFAULT;
...
void CRenderer::SetProjectionMatrix(BOOL perspective){ //set projection matrix 
  D3DXMATRIXA16 matProj;
  if(perspective)D3DXMatrixPerspectiveFovLH(&matProj, D3DX_PI/4.0f,
    (float)g_nScreenWidth/g_nScreenHeight, 1.0f, 10000.0f);
  else D3DXMatrixOrthoOffCenterLH(&matProj, -(float)g_nScreenWidth, (float)g_nScreenWidth, 
    -(float)g_nScreenHeight,(float)g_nScreenHeight, 1.0f, 10000.0f);
...
void CRenderer::SetD3dStates(BOOL perspective){
  ...
  SetProjectionMatrix(perspective); //set projection matrix
...
BOOL CRenderer::RestoreDevice(){ //restore lost device and all surfaces
  ...
  if(FAILED(m_d3ddevice->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO, &m_d3dbackbuffer)) )
    return FALSE;
  SetD3dStates(TRUE); //set D3D default states
